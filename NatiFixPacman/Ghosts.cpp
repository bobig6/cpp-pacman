...
RandomDirection Ghosts::OppositeDirection(RandomDirection dir) //this returns the opposite of the last direction that I have been in
{
    switch (dir) {
...
void Ghosts::Move(float deltaTime, const sf::Vector2f& pacmanPos)
{
    ...
    if (MoveTo(deltaTime)) {
        ...
        // if the ghost is in frightened mode, it has to go to the opposite direction 
...
void Ghosts::SetGhostMode(GhostMode value)
{
    mode = value;
}
